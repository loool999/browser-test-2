...
import { getScreenshot, executeAction } from './browserService';
import { compressBase64 } from '../utils/compression';
import { logger } from '../utils/logger';
import { 
  detectDeviceType, 
  calculateStreamSettings, 
...
const clientStreams = new Map<string, StreamSettings>();
...
export function initializeStream(
  ...
  const sessionId = (socket as any).session?.id;
  
  // Calculate optimal settings based on device and connection
  const streamSettings = calculateStreamSettings(connectionQuality, deviceType);
  ...
  logger.info(`Stream initialized for client ${socket.id}`, {
    browserId,
    fps: settings.fps,
 ...
 */
function setupStreamListeners(socket: Socket): void {
  // Update streaming settings
  socket.on('stream-settings', (settings: Partial<StreamSettings>, callback) => {
    ...
    if (Object.keys(changes).length > 0) {
      logger.info(`Stream settings updated for client ${socket.id}`, changes);
    }
  ...
  socket.on('stream-control', ({ streaming }, callback) => {
    ...
    if (!settings) {
      if (callback) callback({ success: false, error: 'No stream found' });
      return;
    }
    ...
    logger.info(`Stream ${settings.active ? 'resumed' : 'paused'} for client ${socket.id}`);
  ...
  socket.on('latency-report', (data: { latency: number }) => {
    const settings = clientStreams.get(socket.id);
    if (!settings) return;
  ...
  socket.on('disconnect', () => {
    ...
    logger.info(`Stream ended for client ${socket.id} (disconnected)`);
  });
}
 ...
 */
function adjustQualityBasedOnLatency(settings: StreamSettings): void {
  if (!settings.latency) return;
 ...
 */
function startStreamLoop(socket: Socket, settings: StreamSettings): void {
  let isStreaming = true;
  let targetInterval = 1000 / settings.fps;
  
  // Function to get the next frame
  const getNextFrame = async (): Promise<void> => {
    if (!isStreaming || !settings.active) return;
    ...
    try {
      ...
      const screenshot = await getScreenshot(settings.browserId, {
        quality: settings.quality,
        isKeyframe
      });
      ...
      if (screenshot) {
        ...
        if (isKeyframe) {
          const compressedFrame = compressBase64(screenshot);
      ...
      logger.error(`Streaming error for client ${socket.id}:`, error);
      isStreaming = false;
    }
  };
 ...
 */
export function getStreamStats(socketId: string): Record<string, any> | null {
  const settings = clientStreams.get(socketId);
...
export function getAllStreams(): Array<{
  socketId: string;
  stats: ReturnType<typeof getStreamStats>;
}> {
  const streams = [];
 ...
 */
export function getTotalBandwidthUsage(): number {
